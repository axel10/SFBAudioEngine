...
static void SFBCreateDSDDecoderLog(void) { gSFBDSDDecoderLog = os_log_create("org.sbooth.AudioEngine", "DSDDecoder"); }
...
+ (void)load {
    [NSError
            setUserInfoValueProviderForDomain:SFBDSDDecoderErrorDomain
                                         ...
                                         case SFBDSDDecoderErrorCodeDecodingError:
                                             if ([userInfoKey isEqualToString:NSLocalizedDescriptionKey]) {
                                                 return NSLocalizedString(
                                                         @"An error occurred during DSD audio decoding.", @"");
                                             }
                                             break;
    ...
    for (SFBDSDDecoderSubclassInfo *subclassInfo in _registeredSubclasses) {
        ...
        if (detectContentType) {
            ...
            if ([klass testInputSource:inputSource formatIsSupported:&formatSupported error:error]) {
                ...
                os_log_error(gSFBDSDDecoderLog, "Error testing %{public}@ format support for %{public}@", klass,
                             inputSource);
            }
        }
...
- (BOOL)decodeIntoBuffer:(AVAudioBuffer *)buffer error:(NSError **)error {
    ...
    return [self decodeIntoBuffer:(AVAudioCompressedBuffer *)buffer
                      packetCount:((AVAudioCompressedBuffer *)buffer).packetCapacity
                            error:error];
}
